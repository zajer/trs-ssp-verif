...
let _constructed_state2string cs = 
    "(state:"^(cs.Phase3.state.index |> string_of_int)^" ui map:"^(Ui.map_to_string cs.ui_map)^" time:"^(string_of_int cs.time)^" SAT config:"^(Ssp.Template_state.to_stirng cs.sat_config)^")"
...
let test_perform_phase_2 _ = 
    let states_filename = "exmp_2_states.csv"
    and trans_filename = "exmp_2_trans_normed.csv"
    and walk_filename = "exmp_2_walk.csv"
    and number_of_agents = 2 in
    let states = Tracking_bigraph.TTS.import_states states_filename
    and trans = Tracking_bigraph.TTS.import_transitions ~headers_in_first_row:false trans_filename
    and walk = Ssp.Frontend.import_trans_funs walk_filename in
    ...
    and expected_states =
        [
            {
                Phase3.state=(List.nth states 3);
                ui_map=[(1,0);(2,1);(3,2);(4,3);(5,4);(6,5)] |> Ui.make_map_of_list;
                sat_config=[|(1,0);(2,0)|];time=0
            };
            {
                Phase3.state=(List.nth states 1);
                ui_map=[(1,0);(2,2);(3,3);(4,1);(5,4);(6,5)] |> Ui.make_map_of_list;
                sat_config=[|(1,1);(2,0)|];time=1
            };
            {
                Phase3.state=(List.nth states 3);
                ui_map=[(1,0);(2,1);(3,2);(4,3);(5,4);(6,5)] |> Ui.make_map_of_list;
                sat_config=[|(1,2);(2,0)|];time=2
            };
            {
                Phase3.state=(List.nth states 5);
                ui_map=[(5,0);(4,1);(3,2);(2,3);(1,4);(6,5)] |> Ui.make_map_of_list;
                sat_config=[|(1,3);(2,3)|];time=3
            };
            {
                Phase3.state=(List.nth states 2);
                ui_map=[(4,0);(6,1);(2,2);(3,3);(1,4);(5,5)] |> Ui.make_map_of_list;
                sat_config=[|(1,4);(2,4)|];time=4
            }
        ]
     in
    assert_equal
