...
let test_perform_phase_5 _ =
    ...
    assert_equal
        ...
        expected_time_infos
        result_time_infos
let test_perform_phase_6 _ =
    let previous_state = {Tracking_bigraph.TTS.bigraph=Phase3_tests_data.phase_test_two_agentsV2_previous_state;index=0}
    and previous_state_mapping = [(1,0);(2,1);(3,2);(4,3)] |> Ui.make_map_of_list 
    and previous_sat_config = [|(1,0);(2,0)|]
    and ewalk = Phase3_tests_data.phase_test_two_agentsV2_ewalk_1
    and constructed_time_moment = 1
    and num_of_agents = 2
    and all_states = Phase3_tests_data.phase_test_two_agentsV2_all_states
    and all_trans_by_idx = Phase3_tests_data.phase_test_two_agentsV2_all_trans_by_idx
    and all_trans_by_keys = Phase3_tests_data.phase_test_two_agentsV2_all_trans_by_keys () in
    let state_at_previous_moment = {Phase3.state=previous_state;ui_map=previous_state_mapping;sat_config=previous_sat_config;time=constructed_time_moment-1} in
    let result_state,unused_ewalk,result_time_infos = Phase3.perform_phase
                    state_at_previous_moment
                    ewalk
                    ...
                    all_trans_by_idx
                    all_trans_by_keys 
    and expected_unused_walk = [List.nth Phase3_tests_data.phase_test_two_agentsV2_ewalk_1 1]
    and expected_result_state = {Tracking_bigraph.TTS.bigraph=Phase3_tests_data.phase_test_two_agentsV2_middle_state;Tracking_bigraph.TTS.index=1}
    and expected_state_mapping = [(1,0);(2,1);(3,2);(4,3)] |> Ui.make_map_of_list
    and expected_result_sat = [|(1,0);(2,1)|] 
    and expected_time_infos = [{Phase3.participants=[2;3;4]; react_label="PT-2AV2-R1"; transition_idx=1; new_objs=[]; term_objs=[]; start_time=0; end_time=1}] in
    assert_equal
        ~msg:"There should be one unused walk element in the result"
        ~cmp:_compare_ewalks
        ~printer:(fun ew -> "walk of length:"^(string_of_int (List.length ew)))
        expected_unused_walk
        unused_ewalk;
    ...
    assert_equal
        ...
        expected_time_infos
        result_time_infos
let test_perform_phase_7 _ =
    let previous_state = {Tracking_bigraph.TTS.bigraph=Phase3_tests_data.phase_test_two_agentsV2_previous_state;index=0}
    and previous_state_mapping = [(1,0);(2,1);(3,2);(4,3)] |> Ui.make_map_of_list 
    and previous_sat_config = [|(1,0);(2,0)|]
    and ewalk = Phase3_tests_data.phase_test_two_agentsV2_ewalk_1
    and constructed_time_moment = 3
    and num_of_agents = 2
    and all_states = Phase3_tests_data.phase_test_two_agentsV2_all_states
    and all_trans_by_idx = Phase3_tests_data.phase_test_two_agentsV2_all_trans_by_idx
    and all_trans_by_keys = Phase3_tests_data.phase_test_two_agentsV2_all_trans_by_keys () in
    ...
    let result_state,unused_ewalk,result_time_infos = Phase3.perform_phase
                    ...
                    all_trans_by_keys 
    and expected_unused_walk = []
    and expected_result_state = {Tracking_bigraph.TTS.bigraph=Phase3_tests_data.phase_test_two_agentsV2_output_state;Tracking_bigraph.TTS.index=2}
    and expected_state_mapping = [(1,0);(2,1);(3,2);(4,3)] |> Ui.make_map_of_list
    and expected_result_sat = [|(1,3);(2,1)|] 
    and expected_time_infos = 
        [
            {Phase3.participants=[1;3;4]; react_label="PT-2AV2-R2"; transition_idx=2; new_objs=[]; term_objs=[]; start_time=0; end_time=3};
            {Phase3.participants=[2;3;4]; react_label="PT-2AV2-R1"; transition_idx=1; new_objs=[]; term_objs=[]; start_time=0; end_time=1}
        ] in
    assert_equal
        ~msg:"There should be one unused walk element in the result"
        ~cmp:_compare_ewalks
...
let suite =
    "Phase 3" >::: [
        ...
        "Performing phase 3 test 6 - two agents - walk with two elements with different duration time - only one is applicable because reaction ends after currently constructed state ">:: test_perform_phase_6;
        "Performing phase 3 test 7 - two agents - walk with two elements with different duration time - both are applicable">:: test_perform_phase_7
    ]
