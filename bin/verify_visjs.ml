let help () = 
  ...
  Arg5 - the name of a file with patterns that if one of them is detected in one of the contructed states the verified scenario will be marked as invalid.
  Arg6 - a number of agents in the system.
  "
let () = 
    if Array.length Sys.argv = 7 then
        ...
        and patterns_filename = Sys.argv.(5)
        and number_of_agents = Sys.argv.(6) |> int_of_string in
        ...
        let patterns = Ssp_bridge.Parsing.parse_destingation_patterns patterns_filename
        and states = Tracking_bigraph.TTS.import_states states_filename
