...
let help () = 
  ...
  Arg6 - a short name that will work as a prefix for all files associated with this program execution outputs. (no spaces)
  Arg7 - (optional) the name of a file with patterns that if one of them is detected in one of the contructed states the verified scenario will be marked as invalid.
  "
let () = 
    if Array.length Sys.argv >= 7 then
        ...
        and unique_name = Sys.argv.(6)
        and patterns_filename = if Array.length Sys.argv > 7 then Some Sys.argv.(7) else None in
        let raw_config = Yojson.Safe.from_file raw_config_filename |> [%of_yojson: Ssp_verification.Frontend.config_raw] in
        let main_config = Ssp_verification.Frontend.convert raw_config unique_name in
        ...
        and transformer =
            ...
            Ssp_verification.Phase1.ResultTransformer.stack
                (Ssp_verification.Phase1.BasicTransformers.save_or_update_output 
                    init_output 
                    (Ssp_verification.Phase1.output_filename unique_name)
                )
            |>
